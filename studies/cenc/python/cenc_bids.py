...
def participants(in_cenc_dir, in_method, in_filter='study', verbose=False):
     ...
     if in_filter in 'study':
          cenc_id_pattern = 'sub-34P1[0-9][0-9][0-9]'
     elif in_filter in 'test':
          cenc_id_pattern = 'sub-34P9[0-9][0-9][0-9]'
     else:
          bcenc_id_pattern = 'sub-34P[19][0-9][0-9][0-9]'
...
def participant_id(in_dir, cenc_dir=os.getenv('CENC_BIDS_DIR'), acrostic_flag=True, directory_flag=True, exist_flag=True):
     
     pattern = re.compile('34P[19]\d{3}')

     if in_dir.isdigit():
#          print(['Is digit', in_dir])
          in_dir = 'sub-34P1' + '{0:03d}'.format(int(in_dir))
...
#     print(['Is directory', in_dir])
     ...
     cenc_participant_id     = extract_participant_id( in_abs_dir,'sub-34P[19]\d{3}')
...
def directories( input_dir ):
     ...
     freesurfer_dict = { 'base': freesurfer_dir,
                         ...
                         't2w': None, # os.path.join(freesurfer_dir, 'input', 't2tse.nii.gz'),
                         't2w_fatsat': None, #os.path.join(freesurfer_dir, 'input', 't2tse_fatsat.nii.gz'),
                         'flair': None, # os.path.join(freesurfer_dir, 'input', 't2flair.nii.gz'),
