...
BIDSDIR=$INFINITE_BIDS_PATH
MRIQC_OUTPUT_DIR=$INFINITE_MRIQC_PATH
...
for ii in ${PARTICIPANTS}; do
    echo "running mriqc on $ii: output log is in $MRIQC_LOGDIR/inf_mriqc.$ii.output.log"
    nohup time /usr/local/bin/singularity run -w -B /cenc -B /gandg -B /bkraft1 /cenc/software/mriqc/poldracklab_mriqc_latest-2017-08-30-f54388a6fb57.img $BIDSDIR $MRIQC_OUTPUT_DIR \
       --no-sub -w $MRIQC_WORKDIR participant --participant_label $ii &> $MRIQC_LOGDIR/inf_mriqc.$ii.output.log &
done
